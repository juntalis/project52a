...
void inject_x64(LPPROCESS_INFORMATION ppi, const wchar_t* dllpath)
{
	CONTEXT context;
	DWORD   len;
	LPVOID  mem;
...
#define CODESIZE 92
	static BYTE code[CODESIZE+WSIZE(MAX_PATH)] = {
	...
	len = WSIZE(wcslen(dllpath) + 1);
	if (len > WSIZE(MAX_PATH))
		return;
